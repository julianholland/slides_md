...
def _slide_context(slide: SlideConfig, resolver: ImageResolver, strict: bool, warnings: list[str]) -> SimpleNamespace:
    for w in slide.warnings:
        warnings.append(f"slide {slide.index + 1}: {w}")

    body_warnings = render_mod.check_unsupported_body_syntax(slide.body)
    for w in body_warnings:
        msg = f"slide {slide.index + 1}: {w}"
        if strict:
...
def build(
    ...
    deck: DeckConfig = build_deck_config(deck_raw)

    warnings: list[str] = []
    resolver = ImageResolver(images_dir)

    slide_configs = [validate_slide(rs, deck, strict) for rs in raw_slides]
    slide_contexts = [_slide_context(s, resolver, strict, warnings) for s in slide_configs]
    ...
    html = template.render(deck=deck, slides=slide_contexts, layout_templates=TEMPLATE_BY_LAYOUT)
